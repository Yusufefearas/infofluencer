...
def run_youtube_report(report_type, access_token, refresh_token, client_id, client_secret):
    try:
        creds = Credentials(
            token=access_token,
        ...
        dispatch_table = {
            ...
            # buraya diğer fonksiyonları eklersin
        }
...
def run_youtube_report_country(credentials):
    service = build("youtubeAnalytics", "v2", credentials=credentials)
